...
public class Pvm {
    ...
    /**
     * @param p_pvm pvm merkkijonona
     * @return vuosi
     */
    public static int getVuosi(String p_pvm) {
        Pvm pvm = new Pvm(p_pvm);
        return pvm.vv;
    }
    ...
    public static String TarkistaPvm(String pvm) {
        ...
        if (sb.length() > 0)
            return "päivämäärässä ylimääräisiä merkkejä: " + sb.toString();
        if (pv == 0 || kk == 0 || vv == 0)
            return "anna päivämäärä, erota luvut pisteellä";
        if (kk > 12 || kk < 0)
            return "kuukausi on virheellinen";
        int karkaus = 0;
        if (vv % 4 == 0 && vv % 400 != 0)
            karkaus++;
        if (KPITUUDET[karkaus][kk] < pv || pv < 0)
            return "päivä virheellinen";
        if (vv < 0)
            return "vuosi puuttuu tai on virheellinen";
        if (vv < 1990 || vv > 2090)
            return "vuosi on virheellinen";
        return null;
    }
     ...
     *
     */
    public static class Vertailija
            implements Comparator<RekisteroituMerkkijono> {
        ...
        public int compare(RekisteroituMerkkijono o1,
            ...
            if (pvm1.vv - pvm2.vv != 0)
                return pvm2.vv - pvm1.vv;
            if (pvm1.kk - pvm2.kk != 0)
                return pvm2.kk - pvm1.kk;
            if (pvm1.pv - pvm2.pv != 0)
                return pvm2.pv - pvm1.pv;
            return 0;
        }
